import { StyleSheet, View,FlatList, ScrollView,Image, Dimensions,Text, ToastAndroid } from 'react-native'
import React, { useEffect, useState } from 'react'
import { useLocalSearchParams } from 'expo-router'
import { useAddAudioToPlaylist, useGetPlayList, useRenameThePlayList } from '../../hooks/usePlayList'
...
const OnePlayList = () => {
   ...
   let {handleAudioSelectFromPlaylist,allAudioFiles,handleRefreshLibraryPage}=useMusicProvider()
  ...
  let handleAddAudioToPlaylist=async (audioId)=>{
    ...
    let gettingSameFileArray=singlePlayListObj.audios.filter((sig)=>sig.id==audioId)
    if(gettingSameFileArray.length>0){
      setShowModal(!showModal)
      return ToastAndroid.show("Already Added to this playlsit",ToastAndroid.SHORT)
  ...
  return (
    <View style={{backgroundColor:"black",height:Dimensions.get("window").height*.8}}>
          ...
          <ScrollView>
                <LinearGradient style={styles.gradientWrapper} colors={["#f89ef6","black"]}>
                      <Image style={{width:"70%",height:"70%"}} source={playlistCoverPic} />
                      <Text style={{color:"white",fontSize:30,fontWeight:700,padding:5,textAlign:"center"}}>{newName?newName:singlePlayListObj.name}</Text>
                </LinearGradient>
                <View>
                    <CustomPlayListTab playlistName={singlePlayListObj.name} renamePlaylistFunc={renameThePlaylist} playlistAudioArray={singlePlayListObj.audios} playlistId={onePlayListId}/>
                </View>
                <View style={{display:"flex",justifyContent:"center",alignItems:"center",marginTop:"1%"}}>
                    <TouchableOpacity style={styles.libraryBtn} onPress={()=>handleModal()} >
                        <Image style={{width:20,height:20}} source={plusIcon}/>
                        <Text style={{color:"white",fontSize:17,fontWeight:"bold",marginLeft:"5%"}}>Add Songs</Text>
                    </TouchableOpacity>
                </View>
          ...
          <AddAudioInPlaylist  handleModal={handleModal} isModalShow={showModal} handleAddAudioToPlaylist={handleAddAudioToPlaylist}></AddAudioInPlaylist>
    </View>
  )
}
