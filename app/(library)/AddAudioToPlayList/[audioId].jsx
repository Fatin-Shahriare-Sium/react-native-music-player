import { StyleSheet, Text, View ,Dimensions,ToastAndroid,TouchableOpacity,Image,FlatList} from 'react-native'
...
const AddToPlayListRepo = () => {
  ...
  let handleAddAudioToPlayList=async (playListId,playListName,audioId)=>{
      ...
      let filterdArrayOfPlaylist=allPlayLists.filter((sig)=>sig.id==playListId)
      let getExsitenceArray=filterdArrayOfPlaylist[0].audios.filter((sig)=>sig.id==audioId)
      console.log("getExsitenceArray.length>",getExsitenceArray.length);
      
      if(getExsitenceArray.length>0){
        setRefreshPage(!refreshPage)
        return ToastAndroid.show("This song is alraedy added",ToastAndroid.SHORT)
       }else{
      ...
      <View style={styles.addPlayListWrapper}>
         ...
         renderItem={(sig)=>{
          return(
               ...
               <TouchableOpacity style={styles.addPlaylistBtn} onPressOut={()=>router.push("/")} onPress={()=>{handleAddAudioToPlayList(sig.item.id,sig.item.name,audioId)}} >
               <Image source={musicLogo}/>
               <Text style={{color:"white",fontSize:17,fontWeight:"bold",marginLeft:"2%"}}>{sig.item.name}</Text>
              </TouchableOpacity>
              
          )
         }}
         ListEmptyComponent={()=><EmptyComponet message={"You have no playlist 😥"}/>}
