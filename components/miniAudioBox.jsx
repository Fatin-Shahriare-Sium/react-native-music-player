...
const MiniAudioBox = () => {
  ...
  let [audioCurrentStatus, setAudioCurrentStatus] = useState({
    currentDuration: currentAudioFile.activeDuration,
    totalDuration: currentAudioFile.totalDuration,
  });
  let intervalRef = useRef(null);
  let notificationId = useRef(null);
  let [showModal, setShowModal] = useState(false);
  let [currentAudioQueue, setCurrentAudioQueue] = useState([...audioQueue]);
  let [currentAudioObj, setCurrentAudioObj] = useState({
    index: audioIndex,
    uri: audioUri,
    filename: audioTitle,
    audioId: audioId,
  ...
  let [
    ...
    setAudioHasRecentlyFinishedCalledTimes,
  ] = useState(0);
  ...
  useEffect(() => {
    setCurrentAudioObj({
      index: currentAudioFile.index,
      uri: currentAudioFile.uri,
      audioId: currentAudioFile.id,
      filename: currentAudioFile.title,
    });
  }, [currentAudioFile]);
  useEffect(() => {
  ...
  let playNext = async () => {
    ...
    let changeCurrentAudioObj =
      currentAudioQueue[willSuffle ? randomIndex : currentAudioObj.index + 1];
    ...
    setCurrentAudioObj({
      index: indexOfQueue,
      uri: changeCurrentAudioObj.uri,
      filename: changeCurrentAudioObj.filename,
      audioId: changeCurrentAudioObj.id,
    });

    //updating currentAudioFile in storage when next song play
    useStoreCurrentAudioFile({
      title: changeCurrentAudioObj.filename,
      uri: changeCurrentAudioObj.uri,
      id: changeCurrentAudioObj.id,
      index: indexOfQueue,
      activeDuration: 0,
      totalDuration: 0,
    });
    if (soundx.current._loaded == true) {
      ...
      await soundx.current.loadAsync(
        { uri: changeCurrentAudioObj.uri },
        { shouldPlay: true, isLooping: isLoopSingle }
      );
  ...
  useEffect(() => {
    ...
    if (isplaying == true) {
      intervalRef.current = setInterval(() => {
        soundx.current.getStatusAsync().then((res) => {
          if (
            res.positionMillis > 0 &&
            res.positionMillis == res.durationMillis
          ) {
            setAudioHasRecentlyFinishedCalledTimes(
              audioHasRecentlyFinisedCalledTimes + 1
            );
            setIsPlaying(false);
            console.log(res.positionMillis, res.durationMillis);
          } else {
            setAudioCurrentStatus({
              currentDuration: res.positionMillis / 1000,
              totalDuration: res.durationMillis / 1000,
            });
          }
        });
      }, 100);
  ...
  let syncWithAudio = async (syncPosition) => {
    console.log(
      "syncWithAudio",
      audioCurrentStatus.totalDuration * 1000 * syncPosition,
      audioCurrentStatus.totalDuration * 1000
    );
    if (!isplaying) {
      setIsPlaying(true);
    }

    await soundx.current.setPositionAsync(
      audioCurrentStatus.totalDuration * 1000 * syncPosition
    );
  };
  ...
  async function pauseSound() {
    setIsPlaying(false);
    await soundx.current.pauseAsync();
    await useUpdateCurrentAudioFile({
      title: currentAudioObj.filename,
      uri: currentAudioObj.uri,
      id: currentAudioObj.audioId,
      index: currentAudioObj.index,
      activeDuration: audioCurrentStatus.currentDuration,
      totalDuration: audioCurrentStatus.totalDuration,
    });
  ...
  return (
    ...
    >
      <AudioPlayer
        isFav={isFav}
        setAudioAsFav={setAudioAsFav}
        deleteAudioAsFav={deleteAudioAsFav}
        playOnceFunc={handlePlayOnec}
        suffleFunc={handleSuffle}
        loopAllFunc={handleLoopAll}
        singleLoopFunc={handleSingleLoop}
        playPreFuc={playPrevious}
        handleSeek={syncWithAudio}
        modalFuc={handleModal}
        isModalShow={showModal}
        playFuc={playSound}
        pauseFuc={pauseSound}
        audioUpdateStatus={audioCurrentStatus}
        playNextFunc={playNext}
        isPlaying={isplaying}
        audioTitle={currentAudioObj.filename}
      ></AudioPlayer>
      <View>
        {audioCurrentStatus.currentDuration > 0 && (
          <Progress.Bar
            progress={
              audioCurrentStatus.currentDuration /
              audioCurrentStatus.totalDuration
            }
            borderWidth={0}
            borderColor="black"
            color={"white"}
            unfilledColor={"#515050"}
            width={Dimensions.get("window").width}
            height={1}
          />
        )}
        ...
        <View style={styles.miniAudioController}>
          <Pressable onPress={isplaying == true ? pauseSound : playSound}>
            <Image
              ...
              source={isplaying == true ? pauseIcon : playIcon}
            />
          </Pressable>
          <Pressable onPress={isFav == true ? deleteAudioAsFav : setAudioAsFav}>
            <Image
              style={{ width: 20, height: 20 }}
              source={isFav == true ? loveRedIcon : loveIcon}
            />
          </Pressable>
        </View>
      </View>
